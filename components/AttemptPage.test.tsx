...
describe('AttemptPage test-taking experience', () => {
  ...
  it('requires confirmation and prevents duplicate submit requests', async () => { let resolveSubmit!: (value: typeof attempt) => void; vi.mocked(attemptsApi.submit).mockReturnValue(new Promise((resolve) => { resolveSubmit = resolve; })); root = createRoot(container); await renderPage(root); await flush(); await act(async () => { button(container, 'Submit Test').click(); }); const dialogButtons = Array.from(container.querySelectorAll('button')).filter((item) => item.textContent?.trim() === 'Submit Test'); const dialogSubmit = dialogButtons.at(-1) as HTMLButtonElement; await act(async () => { dialogSubmit.click(); }); await waitFor(() => expect(attemptsApi.submit).toHaveBeenCalledTimes(1)); expect(dialogSubmit.disabled).toBe(true); await act(async () => { resolveSubmit(attempt); }); await waitFor(() => expect(push).toHaveBeenCalledWith('/attempt/attempt-1/result')); });
  it('handles 409 ATTEMPT_EXPIRED through the server result', async () => { vi.mocked(attemptsApi.submit).mockRejectedValueOnce(new ApiClientError(409, 'Expired', 'ATTEMPT_EXPIRED')); root = createRoot(container); await renderPage(root); await flush(); await act(async () => { button(container, 'Submit Test').click(); }); const dialogSubmit = Array.from(container.querySelectorAll('button')).filter((item) => item.textContent?.trim() === 'Submit Test').at(-1) as HTMLButtonElement; await act(async () => { dialogSubmit.click(); }); await waitFor(() => expect(attemptsApi.result).toHaveBeenCalledWith('attempt-1')); await waitFor(() => expect(push).toHaveBeenCalledWith('/attempt/attempt-1/result')); });
